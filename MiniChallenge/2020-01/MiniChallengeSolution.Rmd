...
* Be careful with indexes.  If you use `LETTERS[0]` you get `character(0)` in return, which will probably hit you with unexpected results.
...
First, we need to know where each value in `x` is on a count between 1 and 26.  This is a good place to use _remainder division_ using the `%%` operator.
...
The left alpha-digit is a little trickier. We have to be careful about zero-indexes here (such as `LETTERS[0]`). First, we get the decimal value of the left alpha-digit with _integer division_.  Notice that I'm using the `x - 1` transformation again.
